...
| 批次移出計畫 | bulk remove from plan | 交易紀錄多選後從目前計畫移出：只屬於這個計畫的（孤兒交易）整筆刪除，同時掛其他計畫的只拿掉這個標籤 |
| 交易明細匯入 | transaction import | 把券商的歷史成交（CSV 檔或直接貼上的文字）批次轉成交易紀錄；有標題列就依標題對欄位，沒見過的代號會查 Yahoo 自動建成標的 |
