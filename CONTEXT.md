...
| 詞彙 | 英文 / 程式欄位 | 定義 |
| --- | --- | --- |
| 計畫 | portfolio | 一個使用者的完整投資計畫：`{ budget, stocks, transactions, snapshots, rev }`，透過 `GET/PUT /api/portfolio` 整份讀寫 |
| 總預算 | `budget` | 計畫的總投資金額（台幣） |
| 標的 | stock | 一檔要投資的股票：名稱、代號（Yahoo Finance symbol）、市場（台股/美股/韓股）、產業類別、幣別、目標比例 `percent` |
| 目標配置 | target allocation | 各標的的 `percent` 佔比，合計 100%；乘上總預算得到每檔的目標金額 |
| 交易 | transaction | 一筆買入/賣出/配息紀錄：`{ stockId, date, shares, price, twd, kind }`；賣出以負股數表示 |
...
| 快照 | snapshot | 每天 14:00 記錄的整體市值，作為歷史圖的備援資料 |
